...
const cartSlice = createSlice({
  ...
  initialState,
  reducers: {
    /** Adds an NFT to the cart. A no-op if it's already there (NFTs are unique, so quantity is always 1). */
...
export const { addToCart, removeFromCart, checkout, resetCart, hydrateCart } = cartSlice.actions;
